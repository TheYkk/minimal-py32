...
void deeep_sleep(void);

// Linker symbols for memory usage calculation
extern uint32_t _sdata,
    _edata, _sbss, _ebss, _sidata, _estack;
...
static void DEBUG_dump_chip_info(void)
{
    ...
    DEBUG_printf("  RDP:              0x%02x (%s)\n", rdp,
                 rdp == 0xAA ? "Level 0 - no protection" : rdp == 0xCC ? "Level 2 - permanent"
                                                                       : "Level 1 - active");
...
int main(void)
{
    ...
    // Default is usually 115200 baud, 8N1. Pin selection depends on DEBUG_TX define.
    DEBUG_init();

    // Setup
    PIN_output(PIN_LED); // set LED pin as output

    // Pass s_systick correctly (cast may be required depending on your log.h)
    LOG_Init(eLOG_LEVEL_DEBUG, (uint32_t *)&s_systick);
...
void deeep_sleep(void)
{
    // 1. Wait for UART to finish transmitting its shift register
    DLY_ms(20);

    // 2. Start the Low Power Timer FIRST!
    // This relies on SysTick, so SysTick MUST still be running here.
    LPT_shot(2000);

    // 3. Disable SysTick safely so it doesn't instantly interrupt WFE
    uint32_t systick_ctrl = SysTick->CTRL; // Backup SysTick state
    SysTick->CTRL = 0;                     // Disable SysTick
    SCB->ICSR |= SCB_ICSR_PENDSTCLR_Msk;   // Clear any pending SysTick flags

    // 4. Clear other pending interrupts (like UART) so they don't prevent sleep
    NVIC->ICPR[0] = 0xFFFFFFFF;
    EXTI->PR = 0xFFFFFFFF;

    // 5. Enter STOP mode (deep sleep) using WFE
    // SEVONPEND allows the LPTIM pending interrupt to wake the CPU
    // without vectoring to a missing/dummy interrupt handler.
    ...
    // 6. Clear LPTIM interrupt flags
    LPTIM->ICR = LPTIM_ICR_ARRMCF;
    NVIC_ClearPendingIRQ(17); // 17 is LPTIM1_IRQn

// 7. Restore the System Clock
// Waking up from STOP mode automatically defaults the MCU back to the HSI clock.
#if (F_CPU > 24000000) || (SYS_USE_HSE == 1)
    CLK_init();
#endif
