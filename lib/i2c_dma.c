...
void I2C_start(uint8_t addr) {
  ...
  if ((I2C_DMA_CHAN->CCR & DMA_CCR_EN) || (I2C1->CR2 & I2C_CR2_DMAEN)) {
...
void I2C_write(uint8_t data) {
  if (!I2C_wait(&I2C1->SR1, I2C_SR1_TXE)) return;
  I2C1->DR = data;
}
...
uint8_t I2C_read(uint8_t ack) {
  ...
  if (!I2C_wait(&I2C1->SR1, I2C_SR1_RXNE)) return 0;
  return I2C1->DR;
}

// Stop I2C transmission
void I2C_stop(void) {
  if(!I2C_rwflag && I2C_status == I2C_STATUS_OK) {
    if (I2C_wait(&I2C1->SR1, I2C_SR1_BTF))
      I2C1->CR1 |= I2C_CR1_STOP;
  }
}
...
void I2C_writeBuffer(uint8_t* buf, uint16_t len) {
  ...
  I2C_DMA_CHAN->CNDTR = len;
  I2C_DMA_CHAN->CMAR  = (uint32_t)buf;
  I2C_DMA_CHAN->CCR  |= DMA_CCR_EN;
...
{
  uint32_t timeout = I2C_TIMEOUT_LOOPS;
  while ((I2C_DMA_CHAN->CCR & DMA_CCR_EN) || (I2C1->CR2 & I2C_CR2_DMAEN)) {
    uint32_t status = I2C1->SR1;
    if (status & I2C_ERROR_FLAGS) {
      I2C_status = (status & I2C_SR1_AF) ? I2C_STATUS_NACK : I2C_STATUS_BUS_ERROR;
      I2C1->CR2 &= ~I2C_CR2_DMAEN;
      I2C_DMA_CHAN->CCR &= ~DMA_CCR_EN;
      DMA1->IFCR = (uint32_t)0xF << I2C_DMA_SHIFT;
      I2C_abort();
      return 0;
    }
    if (!timeout--) {
...
void I2C_DMA_ISR(void) {
  ...
  uint32_t error_flag = (uint32_t)1 << (I2C_DMA_SHIFT + 3);
...
void I2C_readBuffer(uint8_t* buf, uint16_t len) {
  while (len > 0 && I2C_status == I2C_STATUS_OK) {
    --len;
    *buf++ = I2C_read(len > 0);
  }
  I2C_stop();
